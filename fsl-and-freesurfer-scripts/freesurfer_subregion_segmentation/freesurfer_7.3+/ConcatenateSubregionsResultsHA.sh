...
# Taken from Freesurfer (https://surfer.nmr.mgh.harvard.edu/fswiki/ConcatenateSubregionsResults) but customized to work with stats files that are saved in /mri directory 
# as done by subregion_segmentation command in Freesurfer 7.3+. 
# This script will concatenate stats for all subjects found in SUBJECTS_DIR and output them into one
# common table file. 


# USAGE:
# ./ConcatenateSubregionsResults.sh -f <data_file.txt> [-f <data_file_2.txt>] -o out_dir [-s SUBJECTS_DIR]

# EXAMPLE USAGE:
# To put together all hippocampal subfield volumes for all subjects in the SUBJECTS_DIR for the left hemisphere:
# ./ConcatenateSubregionsResults.sh -f lh.hippoSf.txt -o /path/to/output/ -s /path/to/subjects_dir/
...
for stat_file in ${FILES[@]}; do
    ...
    results_file="$OUTPATH${stat_file%'.txt'}_concat.stats"
    echo ""
        ...
        # If first subjet, write file header
            if [ $WRITE_HEADER == 'yes' ]; then
                ...
                while read line; do
                    ...
                    arr=(`echo ${line}`)
                    header_string="$header_string ${arr[-2]}"
                done < $VolFile
